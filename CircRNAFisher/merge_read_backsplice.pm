...
package merge_read_backsplice;
use List::Util qw/max/;
#use strict;
sub merge_read_backsplice
{
my $range=$_[0];
open(IN,"./output/re_mapped.sam")or die "Can't open file: ./output/re_mapped.sam $!";
#open(OUT,">./output/merged_remapped.txt");
open(OUT,">./output/test_mergedremapped_start_end_notsame");
	...
	if(abs($back_start{$chr}[$i]-$back_start{$chr}[$j])==abs($back_end{$chr}[$i]-$back_end{$chr}[$j]))
	 {
	    ...
	    {
	     print OUT "!!!!!!!!!!!!!!!\n";	    
             print OUT $read{$chr}[$i];
	     print OUT $read{$chr}[$j];
	     print OUT "!!!!!!!!!!!!!!!\n";	    
